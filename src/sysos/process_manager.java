...
public class process_manager {
	...
	public class process {
		...
		public int fork(String nazwa) {
			...
			/*
			 * else { System.out.println("Nie utworzono procesu potomnego!");
			 * 
			 * return -1; }
			 */
		}
		...
		public boolean wait_PID() {
			...
			if (this.s == status.ZOMBIE) {
				System.out.println("Proces z PID: " + this.PID
						+ " nie istnieje, wiec nie mozna wywolac tej metody na jego dziecku.");
			} else {
				if (this.child != null) {
		...
		public boolean exit(int stat) {
			...
			if (this.child != null) {
				if (this.child.s == status.ZOMBIE) {
					...
					System.out.println("Dziecko procesu o PID: " + this.PID + " czyli " + this.child.PID
							+ "zmieniĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąË‡ stan na TERMINATED.");
				}
				for (int i = 0; i < ex.size(); i++) {
					if (ex.get(i).who == this.child.PID) {
						ex.remove(i);
			...
			if (temp >= 0) {
				...
				if (only == false) {
					...
					if (free_m(this.name)) {
						...
						// zmiana wskaĂ„â€šĂ˘â‚¬Â¦Ä‚â€šÄąĹşnikĂ„â€šÄŹĹĽËťÄ‚â€šÄąâ€šw na
						// liĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąĹşcie
						...
						if (this.child != null) {
							...
							p.PPID = 0;
							// doĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąË‡Ă„â€šĂ˘â‚¬ĹľÄ‚Ë�Ă˘â€šÂ¬Ă‚Â¦czenie do dzieci
							// inita
							if (INIT.child != null) {
								// doĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąË‡Ă„â€šĂ˘â‚¬ĹľÄ‚Ë�Ă˘â€šÂ¬Ă‚Â¦czenie do
								// najmĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąË‡odszego
						...
						System.out.println(
								"BĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąË‡Ă„â€šĂ˘â‚¬ĹľÄ‚Ë�Ă˘â€šÂ¬Ă‚Â¦d zwalniania pamiĂ„â€šĂ˘â‚¬ĹľÄ‚Ë�Ă˘â‚¬ĹľĂ‹ďż˝ci!");
					}
				}
				...
				if (free_m(this.name)) {
					...
					ex.add(e);
					System.out.println(
							"Na procesie nie wykonano jeszcze metody wait_PID, wiec zostal dodany do listy procesow ZOMBIE.");
					// System.out.println("Usunieto proces o ID: "+this.PID);
					del = true;
					return del;
				} else {
					System.out.println(
							"Na procesie nie wykonano metody wait_PID, jednak wystĂ„â€šĂ˘â‚¬ĹľÄ‚Ë�Ă˘â€šÂ¬Ă‚Â¦piĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąË‡ bĂ„â€šĂ˘â‚¬Â¦Ä‚Ë�Ă˘â€šÂ¬ÄąË‡Ă„â€šĂ˘â‚¬ĹľÄ‚Ë�Ă˘â€šÂ¬Ă‚Â¦d pamiĂ„â€šĂ˘â‚¬ĹľÄ‚Ë�Ă˘â‚¬ĹľĂ‹ďż˝ci!");
				}
			}
			// NWM MAX
			return false;
		...
		public boolean kill(int pid) {
			if (pid == 0) {
				System.out.println(
						"Ta operacja spowoduje zamkniĂ„â€šÄąĹľcie systemu!\nCzy na pewno chcesz jÄ‚â€šĂ„â€¦ wykonaĂ„â€šĂ‚Â¦?\n1- Tak/0 - Nie");
				int c;
			...
			while (p.next != null) {
				...
				if (p.PID == pid) {
					if (find(pid) == Main.S.runningProcess) {
						...
						if (Main.S.qs.get(find(pid).usrpri).size() < 2)
					...
					if (p.s == status.ZOMBIE) {
						System.out.println(
								"Ten proces jest w stanie ZOMBIE, wiĂ„â€šÄąĹľc usuniĂ„â€šÄąĹľtĂ„â€šÄąĹľ zostanÄ‚â€šĂ„â€¦ tylko jego powiÄ‚â€šĂ„â€¦zania.");
			...
			if (is == true) {
				...
				if (temp >= 0) {
					...
					if (free_m(p.name)) {
						...
						e.res = 0;
						ex.add(e);
						// System.out.println("Na procesie nie wykonano jeszcze metody wait_PID,
						// wiĂ„â€šÄąĹľc zostaÄ‚â€šÄąâ€š dodany do listy procesĂ„â€šÄąâ€šw ZOMBIE.");
						System.out.println("Usunieto proces.");
						del = true;
						return del;
					} else {
						System.out.println(
								"Na procesie nie wykonano metody wait_PID, jednak wystÄ‚â€šĂ„â€¦piÄ‚â€šÄąâ€š bÄ‚â€šÄąâ€šÄ‚â€šĂ„â€¦d pamiĂ„â€šÄąĹľci!");
					}
				}
		...
		public void printPages() {
			...
			for (int i = 0; i < current.pageTable.length; i++) {
				if (current.pageTable[i].isInRam) {
					System.out.println(i + ": " + current.pageTable[i].inWhichFrame);
				}
				else {
